...
import matplotlib.pyplot as plt # 23.11.29 추가
...
class FactTrainer(BaseTrainer):
    ...
    def train(self, loader, st_step=0, max_step=100000, folder_name=None):
        ...
        for batch in tqdm(cyclize(loader)):
            ...
            # if self.step % self.cfg.tb_freq == 0: 
              # save_dir = self.cfg['work_dir'] / folder_name
              # if not os.path.exists(f'{save_dir}/figs/') : os.mkdir(f'{save_dir}/figs/')
              # plt.plot(epoch, losses.pixel_loss.val, label='Training Pixel Loss')
              # plt.xlabel('Epochs')
              # plt.ylabel('Loss')
              # plt.title('Training Pixel Loss Over Epochs')
              # plt.savefig(f'{save_dir}/figs/g_total_epoch{epoch}.png')
              ### 추가 끝 
            ...
            if self.cfg.gpu <= 0:
                # 23.11.29 첨언: 그래프를 더 짧은 주기로 뽑고 싶다면, defaults.yaml 파일의 tb_freq 수를 줄여주세요.
                # if self.step % self.cfg.tb_freq == 0: 
                    # save_dir = self.cfg['work_dir'] / folder_name
                    # self.plot(epoch, losses, discs, stats, save_dir) # 23.11.29 수정 'epoch' 파라미터 추가
                    # 여기 부분 수정 필요
                ...
                if self.step > 0 and self.step % self.cfg.val_freq == 0:
                    ...
                    self.save(loss_dic['g_total'], self.cfg.save, self.cfg.get('save_freq', self.cfg.val_freq),folder_name)
            else:
                pass
    ...
    def infer_comp_ac(self, fact_experts, comp_ids):
        ...
        for _b_comp_id, _logit in zip(binary_comp_ids, ac_logit_c):
            ...
            #pdb.set_trace()
            cids = cids.to(torch.device('cuda'))
            eids = eids.to(torch.device('cuda'))
            acc = T_probs.cuda()[cids, eids].sum() / n_experts
            accs += acc
    ...
    def plot(self, epoch, losses, discs, stats, save_dir):
        
        ## 23.11.29 추가 ##
        # 사용하는 코어가 달라질 경우, 아래 savefig 경로를 본인의 컴퓨터에 맞게 변경해주세요.

        # if not os.path.exists(f'{save_dir}/figs/') : os.mkdir(f'{save_dir}/figs/')
        # 1. Plotting g_total loss
        # plt.plot(epoch, losses.g_total.val, label='Training Loss')
        # plt.xlabel('Epochs')
        # plt.ylabel('Loss')
        # plt.title('Training [g_total] Loss Over Epochs')
        # plt.savefig(f'{save_dir}/figs/g_total_epoch{epoch}.png')
        
        # 2. Plotting pixel loss
        # plt.plot(epoch, losses.pixel.val, label='Training Loss')
        # plt.xlabel('Epochs')
        # plt.ylabel('Loss')
        # plt.title('Training [pixel] Loss Over Epochs')
        # plt.savefig(f'{save_dir}/figs/pixel_epoch{epoch}.png')
