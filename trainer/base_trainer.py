...
class BaseTrainer:
    ...
    def save(self, cur_loss, method, save_freq=None,folder_name=None):
        """
        Args:
            method: all / last
        ...
        ckpt_dir = self.cfg['work_dir'] / folder_name / "checkpoints"
        if not os.path.exists(ckpt_dir) : os.system("mkdir {}".format(ckpt_dir))
        ...
        if step_save:
            ...
            os.system(f"python /content/drive/MyDrive/2023학교/AIKU/2023_2_project2/mxfont-main/inference.py --weight_path {str(step_ckpt_path)} --result_idx {int(self.step)} --folder_name {str(folder_name)}")
            log = "Checkpoint is saved to {}".format(str(step_ckpt_path))
        ...
        if not step_save and last_save:
            ...
            os.system(f"python /content/drive/MyDrive/2023학교/AIKU/2023_2_project2/mxfont-main/inference.py --weight_path {str(last_ckpt_path)} --result_idx {int(self.step)} --folder_name {str(folder_name)}")
            log = "Checkpoint is saved to {}".format(str(last_ckpt_path))
