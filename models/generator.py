...
class Generator(nn.Module):
    ...
    def encode(self, img):
        #img2 = img.detach()
        #img2 = img2[0].squeeze()
        #np_arr0 = np.array(img2.cpu(), dtype=np.uint8)
        #img3 = Image.fromarray(np_arr0.astype(np.uint8))
        #save_path = '/content/drive/MyDrive/2023학교/AIKU/2023_2_project2/mxfont-main/data/ttfs/sample_imgs_debug/'
        #img3.save(save_path+"oiajf.png")
        feats = self.style_enc(img)
        #feats = feats.detach()
        #np_arr = np.array(feats.cpu(), dtype=np.uint8)
        #np_arr2 = np_arr.reshape(128,-1)
        #img0 = Image.fromarray(np_arr.astype(np.uint8))
        #img0.save(save_path+"dlkjf.png")
